        ...
        <tbody>
        {
          data.map((d, i) => (
            <tr key={d.name} className={d.in ? styles.in : ''}>
              <td><Link href={`/mrsr/play/${d.name}`}><a>{d.title}</a></Link></td>
              <td>{d.artist}</td>
              <td>{d.level.easy || "-"}</td>
...
export const getStaticProps: GetStaticProps = async () => {
  const mapNames = readdirSync('./public/maps/mrsr').filter(file => file.endsWith('.json')).map(n => n.slice(0, -5))
  const mapData = []

  for (const mapName of mapNames) {
    const { default: map } = await import(`../../public/maps/mrsr/${mapName}.json`)
